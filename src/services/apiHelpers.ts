...
export function paymentSuccess(payload: {
  ...
  const formData = new FormData();
  formData.append("candidateProfileId", payload.candidateProfileId);
...
// results
export function GetResults(id:any) {
  return server.get(endpoints.getResults+id, { requiresAuth: false });
}
export function getStudentResults(id:any) {
  return server.get(endpoints.getStudentResults+id, { requiresAuth: false });
}
