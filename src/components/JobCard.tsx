...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const handleShare = async () => {
    ...
    const shareText = `Check out this latest job opening!\n\n🧾 ${jobTitle}\n📂 Category: ${jobCategory}\n🎓 Qualification: ${qualification}\n⏰ Last Date: ${lastDate}\n\nApply here: ${jobUrl}`;
    ...
    try {
      ...
      alert("Unable to share this job right now.");
    }
  };
      ...
      <div className="bg-white border border-gray-300 rounded-2xl p-6 shadow-sm hover:shadow-md transition">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3 text-gray-600">
            ...
            />
            <FaHeart
              className="cursor-pointer hover:text-pink-500 transition-transform hover:scale-110"
              title="Save Job"
            />
          </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={handleDownload}
              className="bg-pink-500 text-white text-sm px-4 py-2 rounded-md flex items-center gap-1 hover:bg-pink-600 transition"
            >
              <FaDownload /> Notification
            </button>
            ...
            <button
              onClick={() => setOpen(true)}
              className="bg-[#001F5C] text-white text-sm px-4 py-2 rounded-md hover:bg-[#003399] transition"
            >
              View More
            </button>
          </div>
